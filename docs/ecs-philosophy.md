...
- `WeaponComponent`: **данные оружия, лежащего на земле**. Содержит спрайты, скорости, множители скорости передвижения и т.д. Никогда не находится на игроке.
- `EquippedWeaponComponent`: **данные оружия, экипированного сущностью**. Его поля заполняются из `WeaponDefinition` при подборе. Пустые поля означают отсутствие оружия.
